...
export function SearchModal({ open, onClose, onSelectFolder, onSelectDocument }: Props) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<SearchResults>({ folders: [], documents: [] });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] bg-black/50" onClick={onClose}>
      <div
        className="bg-neutral-800 border border-neutral-700 rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center gap-2 px-3 border-b border-neutral-700">
          ...
          <input
            ...
            className="w-full py-3 bg-transparent text-sm text-neutral-200 placeholder:text-neutral-500 focus:outline-none"
          />
          <kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium text-neutral-500 bg-neutral-700/50 rounded border border-neutral-600">
            ESC
          </kbd>
        </div>
          ...
          {!loading && allItems.length > 0 && (
            <div className="py-1">
              {results.folders.length > 0 && (
                <>
                  <p className="px-3 py-1.5 text-[11px] font-medium text-neutral-500 uppercase tracking-wider">
                    Folders
                  </p>
                  {results.folders.map((folder, i) => (
                      <button
                        ...
                        onClick={() => handleSelect({ type: "folder", item: folder })}
                  ...
                  {results.documents.map((doc, i) => {
                    ...
                    return (
                      <button
                        key={doc.id}
                        className={`flex items-center gap-2 w-full px-3 py-2 text-left transition-colors ${
                          activeIndex === idx ? "bg-white/[0.08]" : "hover:bg-white/[0.04]"
                        }`}
                        onClick={() => handleSelect({ type: "document", item: doc })}
                        onMouseEnter={() => setActiveIndex(idx)}
                      >
                        <FileText className="w-4 h-4 text-neutral-500 shrink-0" />
                        <span className="text-sm text-neutral-300 truncate">{doc.name}</span>
                      </button>
                    );
                  })}
                </>
